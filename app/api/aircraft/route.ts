...
async function loadAircraft(): Promise<AircraftPayload> {
  const { lat, lon, radiusNm, listLimit, cacheTtlMs, requestTimeoutMs } = aircraftConfig();
  const now = Date.now();
  ...
  inflight = (async () => {
    try {
      const json = (await fetchAircraftFromUpstream(lat, lon, radiusNm, requestTimeoutMs)) as Parameters<
        typeof parseAircraftResponse
      >[0];
      const parsed = parseAircraftResponse(json, lat, lon, listLimit);
      const data = buildPayload(parsed, radiusNm);
